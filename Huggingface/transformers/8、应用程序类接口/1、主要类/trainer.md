...
在 PyTorch [`Trainer`] 中自定义训练循环行为的另一种方法是使用 [callbacks](callback.md)，这些回调可以检查训练循环状态（用于进度报告、在 TensorBoard 或其他 ML 平台上记录日志等）并做出决策（比如提前停止）。
...
你不需要使用 Accelerate 或 [Deepspeed 集成](deepspeed.md) 功能来使用这些启动器。
