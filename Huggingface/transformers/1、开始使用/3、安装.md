...
#如果需要安装GPU版本的tensorflow和transformer
pip install tensorflow-gpu 'transformers[tf-gpu]'
```
M1 / ARM用户
...
#### 修改缓存设置的方法：
#### 方式一：设置环境变量（推荐）
![[修改hugging face缓存路径方式一.png]]
**新建环境变量之后需要重启系统**
#### 方式二：在调用from_pretrained函数时，使用cache_dir，指定文件夹
```python 
from transformers import pipeline,AutoModel

AutoModel.from_pretrained(
						  "bert-base-uncased",
						  cache_dir=r"F:\Python\HuggingFace_models"
						  )
```
