...
```
![[Pasted image 20240710203643.png]]
```bash
#建议安装gpu版本的tensorflow
...
### AutoModel 
🤗 Transformers 提供了一种简单统一的方式来加载预训练的实例. 这表示你可以像加载 [`AutoTokenizer`] 一样加载 [`AutoModel`]。**唯一不同的地方是为你的任务选择正确的[`AutoModel`]** 。对于文本（或序列）分类，你应该加载 **(哪些任务对应哪些AutoModel????)** [`AutoModelForSequenceClassification`]：
...
```
**注意：`from transformers import AutoModel`、`from transformers import TFAutoModel`，这种方式好像不行。**
## 自定义模型构建
